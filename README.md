...
```sh
apt update -y && apt install -y ffmpeg git clang make && if ! command -v v >/dev/null 2>&1; then git clone --depth=1 https://github.com/vlang/v && cd v && make && ./v symlink && cd ..; fi && git clone --depth=1 https://github.com/tailsmails/stripshot && cd stripshot && v -prod stripshot.v -o stripshot && ln -sf $(pwd)/stripshot $PREFIX/bin/stripshot && stripshot
```
